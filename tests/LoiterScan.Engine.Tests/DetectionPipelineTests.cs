...
/// All propagation is faked so tests are purely algorithmic — no SGP4 needed here.
/// </summary>
public class DetectionPipelineTests
{
    ...
    // ── Test 1: loitering pair is detected ───────────────────────────────────

    [Fact]
    public async Task Cascade_DetectsKnownLoiterPair()
    {
        // A and B are always 3 km apart → well within 5 km threshold for the full horizon
        ...
        var events = await pipeline.RunAsync(MakeConfig(horizonDays: 1), T0);

        Assert.Single(events);
        var ev = events[0];
        Assert.Equal(new PairKey(1, 2), ev.Pair);
        Assert.InRange(ev.MinRangeKm, 0, 5);
        Assert.True(ev.DurationMinutes >= 60, $"Expected ≥ 60 min, got {ev.DurationMinutes}");
    }

    // ── Test 2: flyby (< 1 h close) is NOT detected ──────────────────────────
    ...
    // ── Test 3: bridged excursion extending the qualifying span ──────────────

    [Fact]
    public async Task Cascade_BridgesShortExcursion_AndDetectsEvent()
    {
        // E and F are within 3 km for 55 min, then 4-min excursion > 5 km, then 10 more min ≤ 5 km
        // Total qualifying span ≥ 60 min (55 + 4 bridge + 10 = 69 min elapsed → event)
        ...
        var propagator = new FakePropagator(new Dictionary<DateTime, Func<DateTime, (double, double, double)>>
        {
            ...
            [epochF] = at =>
            {
                ...
                if (m < 59)  return (7010, 0, 0);   // 4-min excursion (≤ 5-min allowance → bridged)
        ...
        Assert.Single(events);
        Assert.True(events[0].DurationMinutes >= 60);
    }

    // ── Test 4: long excursion resets the timer (no event) ───────────────────

    [Fact]
    public async Task Cascade_LongExcursionResetsTimer_NoEvent()
    {
        // G and H are within 5 km for 30 min, then 10-min excursion (> 5-min allowance → reset),
        ...
        var propagator = new FakePropagator(new Dictionary<DateTime, Func<DateTime, (double, double, double)>>
        {
            ...
            [epochH] = at =>
            {
                ...
                if (m >= 30 && m < 40) return (7060, 0, 0);   // 10-min excursion → timer reset
                if (m >= 40 && m < 90) return (7003, 0, 0);   // 50 min in range (< 60)
                return                        (7200, 0, 0);   // far (200 km) — times before T0 and after T0+90min
            },
        });
    ...
    public async Task Cascade_ExcludesDebrisWhenToggleOn()
    {
        ...
        var events = await pipeline.RunAsync(config, T0);

        Assert.Empty(events);  // X filtered out → no pair → no event
    }

    // ── Test 6: apogee/perigee gating eliminates impossible pairs ────────────
    ...
    public async Task Cascade_ApogeePerigeeGating_EliminatesImpossiblePair()
    {
        ...
        // Difference > 50 km coarse threshold → gated out before any propagation.
    ...
    // ── Test 7: coarse-pass performance sanity check ─────────────────────────
    ...
    public async Task Cascade_CoarsePass_CompletesInReasonableTime_For500Objects()
    {
        ...
        // Angular separation ≈ 1.6° ≈ 1100 km — well above the 50 km coarse threshold.
        ...
        var events = await pipeline.RunAsync(MakeConfig(horizonDays: 7), T0);
        sw.Stop();

        Assert.Empty(events);                                          // no close pairs
        Assert.True(sw.Elapsed.TotalSeconds < 15,
            $"Coarse pass took {sw.Elapsed.TotalSeconds:F1}s — expected < 15s");
    }
}
